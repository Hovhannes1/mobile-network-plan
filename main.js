...
function scanArea() {
  ...
  for (
    col = 0;
    col < areaWidth;
    col++ //for each column in the current block
  ) {
...
function isPointInPolygon(location) {
  // check if not out of polygon bounds
  if (
    location.lat > mainPolygonZone.getBounds().getNorth() ||
    location.lat < mainPolygonZone.getBounds().getSouth() ||
    location.lng > mainPolygonZone.getBounds().getEast() ||
    location.lng < mainPolygonZone.getBounds().getWest()
  ) {
    return false;
  } else {
    return true;
